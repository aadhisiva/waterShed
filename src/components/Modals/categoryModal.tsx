...
export default function CategoryModal({
  ...
  const [activityOptions, setActivityOptions] = React.useState<any>([]);
  ...
  const fecthIntialData = async () => {
    ...
    let response = await axiosInstance.post('addOrGetsCategory', {
      ReqType: 'Dd',
    });
    let activityRes = await axiosInstance.post('addOrGetSectors', {
      ReqType: 'Dd',
    });
    if (data?.code == 200) {
      setDepartmentOptions(data.data);
      setActivityOptions(activityRes.data.data);
      setActivityOption([
        ...response.data.data,
