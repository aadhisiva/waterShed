...
export const mobileAppResponse = async (res, result, body = {}, logRes={userId: '', logMessage: ''}) => {
    const { code=500, message, data } = result;
    const { userId,logMessage } = logRes;
    if (result instanceof Error) {
        await saveMobileLogs(logMessage, userId, body, result, RESPONSETYPE.FAILED)
        ...
        await saveMobileLogs(logMessage, userId, body, result, RESPONSETYPE.SUCCESS)
        if (!data) {
...
export const mobileAppResponseForLarge = async (res, result, body = {}, logRes={userId: '', logMessage: ''}) => {
    const { code, message, data } = result;
    const { userId, logMessage } = logRes;
    if (result instanceof Error) {
        await saveMobileLogs(logMessage, userId, body, result, RESPONSETYPE.FAILED)
        ...
        await saveMobileLogs(logMessage, userId, body, logMessage, RESPONSETYPE.SUCCESS)
        if (!data) {
            return res.status(200).send(ResponseMessages(ResponseCode.SUCCESS, (message || RESPONSEMSG.RETRIVE_SUCCESS), result));
        }
        return res.status(200).send(ResponseMessages(ResponseCode.SUCCESS, (message || RESPONSEMSG.RETRIVE_SUCCESS), data));
...
export const webAppResponseForLarge = async (res, result, body = {}, page = '', msg = '', userid = '', role = '') => {
    const { code, message, data } = result;
    if (result instanceof Error) {
        await saveWebLogs(page, msg, userid, body, result, RESPONSETYPE.FAILED)
        return res.status(500).send(ResponseMessages(ResponseCode.EXCEPTION, (message || RESPONSEMSG.EXCEPTION), RESPONSE_EMPTY_DATA));
    } else if (code == 422) {
        await saveWebLogs(page, msg, userid, body, result, RESPONSETYPE.FAILED)
        return res.status(code).send(ResponseMessages(ResponseCode.UNPROCESS, (message || RESPONSEMSG.UNPROCESS), RESPONSE_EMPTY_DATA));
        ...
        await saveWebLogs(page, msg, userid, body, msg, RESPONSETYPE.SUCCESS)
        if (!data) {
            return res.status(200).send(ResponseMessages(ResponseCode.SUCCESS, (message || RESPONSEMSG.RETRIVE_SUCCESS), result));
        }
        return res.status(200).send(ResponseMessages(ResponseCode.SUCCESS, (message || RESPONSEMSG.RETRIVE_SUCCESS), data));
...
export const webAppResponse = async (res, result, body = {}, page = '', msg = '', userid = '', role = '') => {
    ...
    if (result instanceof Error) {
        ...
        await saveWebLogs(page, msg, userid, body, result, RESPONSETYPE.SUCCESS)
        if (!data) {
            return res.status(200).send(ResponseMessages(ResponseCode.SUCCESS, (message || RESPONSEMSG.RETRIVE_SUCCESS), result));
        }
        return res.status(200).send(ResponseMessages(ResponseCode.SUCCESS, (message || RESPONSEMSG.RETRIVE_SUCCESS), data));
